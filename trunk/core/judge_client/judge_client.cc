...
void write_log(const char *_fmt, ...) {
    ...
    if (DEBUG) {
        // printf("%s\n", buffer);
        DebugLog("%s", buffer); // DebugLog will add extra '\n'
    }
...
void init_syscalls_limits(int lang) {
    ...
    if (DEBUG) {
        DebugLog("init_syscalls_limits: lang=%d", lang);
    }
...
int compile(int lang, char *work_dir) {
    ...
    const char *CP_GO[] = { "/usr/lib/go-1.6/bin/go", "build", "-o", "Main", "Main.go", NULL };
...
void copy_go_runtime(char *work_dir) {
    ...
    // execute_cmd("/bin/cp /usr/lib/go-1.6/bin/go %s/usr/bin/", work_dir);
    putenv((char *)"GOPATH=/usr/lib/go-1.6");
...
void run_solution(int &lang, char *work_dir, int &time_lmt, int &usedtime,
    ...
    DebugLog("[%s:%d] lang=%d ", __func__, __LINE__, lang );
    ...
    switch (lang) {
    case EL_LANG_GOLANG:
    case EL_LANG_CSHARP: //C#
        LIM.rlim_cur = LIM.rlim_max = 280;
        break;

    case EL_LANG_JAVA:  //java
    case EL_LANG_RUBY:  //ruby
...
int main(int argc, char **argv) {
    ...
    if (lang == EL_LANG_RUBY) {
        copy_ruby_runtime(work_dir);
    }

    if (lang == EL_LANG_BASH) {
        copy_bash_runtime(work_dir);
    }

    if (lang == EL_LANG_PYTHON) {
    ...
    if (lang == EL_LANG_PERL) {
        copy_perl_runtime(work_dir);
    }

    if (lang == EL_LANG_CSHARP) {
        copy_mono_runtime(work_dir);
    }

    if (lang == EL_LANG_OBJC) {
        copy_objc_runtime(work_dir);
    }

    if (lang == EL_LANG_FREEBASIC) {
        copy_freebasic_runtime(work_dir);
    }

    if (lang == EL_LANG_SCHEME) {
    ...
    if (DEBUG) {
        int rcode = oi_mode ? finalACflg : ACflg;
        write_log("result=%s\n", OJ_DESC[rcode]);
    }
